...
export async function checkIn(name) {
  ...
  must(error);
  await sb().from('check_in_events').insert({});
...
export async function presentSingles(excludeId) {
  ...
  const { data, error } = await sb().from('players').select('id,name,checked_in_at')
...
export async function formDuo(playerId, partnerId) {
  const { data: ps, error } = await sb().from('players').select('*').in('id', [playerId, partnerId]);
  must(error);
  ...
  const { data: duo, error: e2 } = await sb().from('duos')
    .insert({ name1: me.name, name2: partner.name, p1: me.id, p2: partner.id, status: 'queued' })
    .select().single();
  must(e2);
  await sb().from('players').update({ duo_id: duo.id }).in('id', [me.id, partner.id]);
  return { id: duo.id, names: [duo.name1, duo.name2] };
}

export async function disbandDuo(duoId) {
  const { data: duo } = await sb().from('duos').select('*').eq('id', duoId).maybeSingle();
  if (!duo) return;
  if (duo.status === 'playing') throw new Error('A dupla está jogando; encerre o jogo pela recepção.');
  await sb().from('players').update({ duo_id: null }).in('id', [duo.p1, duo.p2].filter(Boolean));
  await sb().from('courts').update({ called_duo_id: null, status: 'free' }).eq('called_duo_id', duoId);
  await sb().from('duos').delete().eq('id', duoId);
}
...
export async function removeSchedule(scheduleId) {
  await sb().from('schedule').delete().eq('id', scheduleId);
}
...
async function getCourt(courtId) {
  const { data } = await sb().from('courts').select('*').eq('id', courtId).maybeSingle();
  if (!data) throw new Error('Quadra não encontrada.');
  return data;
}
async function queueHead() {
  const { data } = await sb().from('duos').select('*').eq('status', 'queued').order('formed_at', { ascending: true }).limit(1);
  return data?.[0] || null;
}
...
export async function callNext(courtId) {
  ...
  const { data: rows } = await sb().from('schedule').select('*').eq('court_id', courtId);
  ...
  await sb().from('duos').update({ status: 'called', court_id: courtId }).eq('id', head.id);
  await sb().from('courts').update({ status: 'prep', called_duo_id: head.id, duo_id: null, play_started_at: null }).eq('id', courtId);
}
...
export async function startGame(courtId) {
  ...
  await sb().from('duos').update({ status: 'playing' }).eq('id', c.called_duo_id);
  await sb().from('courts').update({ status: 'playing', duo_id: c.called_duo_id, called_duo_id: null, play_started_at: new Date().toISOString() }).eq('id', courtId);
}

export async function endGame(courtId) {
  const c = await getCourt(courtId);
  if (c.status === 'playing' && c.duo_id) {
    const { data: duo } = await sb().from('duos').select('*').eq('id', c.duo_id).maybeSingle();
    if (duo) {
      await sb().from('duos').update({ status: 'done' }).eq('id', duo.id);
      await sb().from('players').update({ duo_id: null }).in('id', [duo.p1, duo.p2].filter(Boolean));
    }
  }
  const head = await queueHead();
  if (head) {
    ...
    await sb().from('courts').update({ status: 'free', called_duo_id: null, duo_id: null, play_started_at: null }).eq('id', courtId);
  }
}
...
export async function extendGame(courtId, minutes = 15) {
  ...
  if (c.status === 'playing' && c.play_started_at) {
    ...
    await sb().from('courts').update({ play_started_at: next }).eq('id', courtId);
  }
}
...
export async function blockLesson(courtId, label, minutesUntil = 60) {
  ...
  await sb().from('courts').update({ status: 'lesson', lesson_label: label || 'Aula', lesson_until: until, duo_id: null, called_duo_id: null, play_started_at: null }).eq('id', courtId);
}

export async function unblockLesson(courtId) {
  await sb().from('courts').update({ status: 'free', lesson_label: null, lesson_until: null }).eq('id', courtId).eq('status', 'lesson');
}

export async function resetAll() {
  await sb().from('check_in_events').delete().neq('id', 0);
  await sb().from('schedule').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  await sb().from('players').update({ duo_id: null }).not('id', 'is', null);
  await sb().from('duos').delete().not('id', 'is', null);
  await sb().from('players').delete().not('id', 'is', null);
  await sb().from('courts').update({ status: 'free', duo_id: null, called_duo_id: null, play_started_at: null, lesson_label: null, lesson_until: null }).not('id', 'is', null);
}
...
export async function snapshot() {
  ...
  const [courtsR, duosR, schedR, evR] = await Promise.all([
    sb().from('courts').select('*').order('sort', { ascending: true }),
    sb().from('duos').select('*'),
    sb().from('schedule').select('*'),
    sb().from('check_in_events').select('at').gt('at', new Date(now - 21 * 24 * 3600 * 1000).toISOString()),
  ]);
